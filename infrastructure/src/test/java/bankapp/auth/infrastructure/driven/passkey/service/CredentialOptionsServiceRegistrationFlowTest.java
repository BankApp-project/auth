...
class CredentialOptionsServiceRegistrationFlowTest {
    ...
    @BeforeEach
    void setup() {
        var passkeyOptionsProperties = new PasskeyRpProperties(
                DEFAULT_RPID,
                DEFAULT_AUTH_MODE
        );

        passkeyOptionsService = new PasskeyOptionsService(
    ...
    void should_return_response_with_default_settings_when_DEFAULT_AUTH_MODE_flag_is_default() {
        ...
        var passkeyOptionsService = new PasskeyOptionsService(
                passkeyOptionsProperties,
                DEFAULT_CLOCK
        );
